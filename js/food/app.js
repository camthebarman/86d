/* food/app.js — the Food tool: ingredients, recipes, inventory, insights,
   usage and catering events. Renders into #mod-food only; the modal and the
...
const FoodApp = (function () {
  ...
  function renderInsights() {
    const panel = $("#food-panel-insights");
    panel.innerHTML = "";
    panel.append(
      el("div", { class: "panel-head" }, [
        el("div", {}, [
    ...
    const ranked = FoodCalc.popularityRanking(state.recipes, getIngredient);
    ...
    panel.append(
      el("div", { class: "grid grid-4" }, [
        insightStat("Most Popular", best ? best.recipe.name : "—", best ? `${FoodCalc.fmtNum(best.servings, 0)} / week` : ""),
        insightStat("Least Popular", worst ? worst.recipe.name : "—", worst ? `${FoodCalc.fmtNum(worst.servings, 0)} / week` : ""),
        insightStat("Biggest Weekly Profit", topProfit ? topProfit.recipe.name : "—", topProfit ? FoodCalc.fmtMoney(topProfit.weeklyProfit) + " / week" : ""),
        insightStat("Highest Food Cost", worstPct ? worstPct.recipe.name : "—", worstPct ? FoodCalc.fmtPct(worstPct.foodCostPct) + " of price" : ""),
      ])
    );
    ...
    ranked.forEach((row, idx) => {
      ...
      tbody.append(
        el("tr", {}, [
          ...
          el("td", { class: "num" }, [FoodCalc.fmtNum(row.servings, 0)]),
          el("td", {}, [
            el("div", { class: "bar" }, [
              el("div", { class: "bar-fill", style: `width:${(row.servings / maxServings) * 100}%` }),
            ]),
            el("div", { class: "muted small-note" }, [FoodCalc.fmtPct(row.share)]),
          ]),
          el("td", { class: "num" }, [FoodCalc.fmtMoney(row.cost)]),
          el("td", { class: "num" }, [row.recipe.menuPrice ? FoodCalc.fmtMoney(row.recipe.menuPrice) : "—"]),
          el("td", { class: "num" }, [row.foodCostPct === null ? "—" : FoodCalc.fmtPct(row.foodCostPct)]),
          el("td", { class: "num" }, [row.recipe.menuPrice ? FoodCalc.fmtMoney(row.profitEach) : "—"]),
          el("td", { class: "num" }, [el("strong", {}, [FoodCalc.fmtMoney(row.weeklyProfit)])]),
        ])
      );
    });
    table.append(tbody);
  ...
  function renderMenuEngineeringCard() {
    const { rows, medServings, medProfit } = FoodCalc.menuEngineering(state.recipes, getIngredient);
    const card = el("div", { class: "card" }, [
      el("h3", {}, ["Menu Engineering"]),
      el("div", { class: "sub", style: "margin-bottom:12px" }, [
        `Every priced dish sorted on two axes: how often it sells and how much it earns per plate. The split is the menu median — ${FoodCalc.fmtNum(medServings, 0)} servings a week and ${FoodCalc.fmtMoney(medProfit)} profit per plate.`,
      ]),
    ]);
    if (!rows.length) {
    ...
    ["star", "plowhorse", "puzzle", "dog"].forEach((cls) => {
      const meta = FoodCalc.MENU_CLASSES[cls];
        ...
        members.forEach((m) => {
          list.append(
            el("li", {}, [
              ...
              el("span", { class: "muted" }, [`${FoodCalc.fmtNum(m.servings, 0)}/wk · ${FoodCalc.fmtMoney(m.profitEach)}/plate`]),
            ])
          );
        });
        box.append(list);
  ...
  function renderSharedIngredientsCard() {
    const pairs = FoodCalc.sharedIngredientPairs(state.recipes, getIngredient, 3).slice(0, 12);
    const card = el("div", { class: "card" }, [
    ...
    pairs.forEach((pair) => {
      tbody.append(
        el("tr", {}, [
          ...
          el("td", { class: "num" }, [FoodCalc.fmtPct(pair.overlapPct)]),
          el("td", {}, [
  ...
  function renderStandaloneCard() {
    const rows = FoodCalc.crossUtilization(state.recipes, getIngredient);
    const card = el("div", { class: "card" }, [
      ...
      tbody.append(
        el("tr", { class: row.sharedPct === 0 ? "row-bad" : lonely ? "row-warn" : "" }, [
          ...
          el("td", {}, [
            ...
            el("div", { class: "muted small-note" }, [`${FoodCalc.fmtPct(row.sharedPct)} shared`]),
          ]),
          el("td", {}, [
  ...
  function renderOrphanCard() {
    const orphans = FoodCalc.orphanIngredients(state.recipes, state.ingredients);
    const tied = orphans.reduce((sum, o) => sum + o.onHandValue, 0);
    const card = el("div", { class: "card" }, [
      el("h3", {}, ["Single-Use Ingredients"]),
      el("div", { class: "sub", style: "margin-bottom:10px" }, [
        orphans.length
          ? `${orphans.length} ingredient${orphans.length === 1 ? "" : "s"} only one dish uses, holding ${FoodCalc.fmtMoney(tied)} of inventory. Pull that dish and this stock has nowhere to go.`
    ...
    orphans.slice(0, 12).forEach((o) => {
      tbody.append(
        el("tr", {}, [
          ...
          el("td", { class: "num" }, [FoodCalc.fmtMoney(o.onHandValue)]),
        ])
      );
    });
    table.append(tbody);
  ...
  function renderWorkhorseCard() {
    const workhorses = FoodCalc.workhorseIngredients(state.recipes, state.ingredients, 10);
    const card = el("div", { class: "card" }, [
    ...
    workhorses.forEach((w) => {
      tbody.append(
        el("tr", {}, [
          ...
          el("td", { class: "num" }, [`${FoodCalc.fmtMoney(FoodCalc.costPerUsableUnit(w.ingredient))} / ${FoodCalc.unitLabel(w.ingredient, 1)}`]),
        ])
      );
    });
    table.append(tbody);
